    ...
    Simd<[T; LANES]>: SimdWrapper<T>,
{
    ...
    pub(crate) fn update_centroid_distances(&self, state: &mut KMeansState<T>) {
        ...
        #[cfg(not(feature = "rayon"))]
        let threads = 1;
        let work_packet_size = self.p_samples.len() / self.p_sample_dims / threads;
        self.p_samples
            .as_slice()
            .par_chunks(self.p_sample_dims)
            .with_min_len(work_packet_size)
            .zip(state.assignments.as_slice().par_iter().cloned())
            .zip(state.centroid_distances.as_mut_slice().par_iter_mut())
            .for_each(|((s, assignment), centroid_dist)| {
                ...
                *centroid_dist =
                    s.chunks_exact(LANES)
                        ...
                        .zip(centroid.chunks_exact(LANES).map(|i| unsafe {
    ...
    pub(crate) fn update_cluster_assignments(
        &self,
        ...
        self.p_samples
            ...
            .par_chunks(self.p_sample_dims)
            .with_min_len(work_packet_size)
            .zip(state.assignments.as_mut_slice().par_iter_mut())
            .zip(state.centroid_distances.as_mut_slice().par_iter_mut())
            .for_each(|((s, assignment), centroid_dist)| {
...
mod tests {
        ...
        Simd<[T; LANES]>: SimdWrapper<T>,
    {
        ...
        samples
            .iter_mut()
            .for_each(|i| *i = thread_rng().gen_range(T::zero(), T::one()));
        ...
        for i in 0..should_assignments.len() {
            assert_approx_eq!(
                state.centroid_distances[i],
                should_centroid_distances[i],
                max_diff
            );
        }
    ...
    #[bench]
    fn distance_matrix_calculation_benchmark_f64(b: &mut Bencher) {
        distance_matrix_calculation_benchmark::<f64>(b);
    }
    #[bench]
    fn distance_matrix_calculation_benchmark_f32(b: &mut Bencher) {
        distance_matrix_calculation_benchmark::<f32>(b);
    }

    fn distance_matrix_calculation_benchmark<T: Primitive>(b: &mut Bencher)
    where
        [T; LANES]: SimdArray,
        Simd<[T; LANES]>: SimdWrapper<T>,
    {
        ...
        samples
            .iter_mut()
            .for_each(|v| *v = thread_rng().gen_range(T::zero(), T::one()));
        let kmean = KMeans::new(samples, sample_cnt, sample_dims);
